#!/usr/bin/python
description = "makes sanity check plots for timeUntilObservable algorithm"
...
for i in xrange(ndetect):
    ...
    filename = 'sanityCheck_timeUntilObservable-%d.txt'%(i)
    print "  "+filename
    ...
    delaytime = np.zeros((npix,), dtype=float)
    for ipix, lat, lon in zip(pix, obsLat, obsPhi):
        if verbose:
            print "      lat=%.1f\tlon=%.1f"%(lat*utils.rad2deg, lon*utils.rad2deg)
        delaytime[ipix] = utils.timeUntilObservable( lat, lon, this_srcDec, this_srcRA, sunDec, sunRA, t, obsAng, solarOcclusionAng=solarOcclusionAng )
        print >> file_obj, "%3.1f %3.1f %8.3f"%(lat*utils.rad2deg, lon*utils.rad2deg, delaytime[ipix]) + "\t%d"%(np.cos(obsAng+solarOcclusionAng) <= np.cos(utils.pi2-lat)*np.cos(sunTheta) + np.sin(utils.pi2-lat)*np.sin(sunTheta)*np.cos(lon-sunPhi))

    file_obj.close()

    #--------------------

    figname = 'sanityCheck_timeUntilObservable-%d.png'%(i)
    print "  "+figname
    ...
    ### plot delay time
    lalinf_plot.healpix_heatmap( delaytime/3600., cmap=plt.get_cmap('OrRd'), nest=False, vmin=0., vmax=24. )
    ...
    mask = delaytime==np.infty ### places that can never observe the true source
    ...
    figname = 'sanityCheck_timeUntilObservable_SolarOcclusion-%d.png'%(i)
    print "  "+figname
    ...
    figname = "sanityCheck_timeUntilObservable_ObservableRegion-%d.png"%(i)
    print "  "+figname
