#!/usr/bin/python
description = "makes sanity check plots for marge_zenithAng algorithm"
...
for i in xrange(ndetect):
    ...
    filename = 'sanityCheck_marge_zenithAng-%d.txt'%(i)
    print "  "+filename
    ...
    print >> file_obj, "%5s %12s"%('lat', 'delay')

    print "    iterating over observatories"
    zenithAng = np.zeros((npix,), dtype=complex)
    for lat in set_obsLat:
        if verbose:
            print "      lat=%.1f"%(lat*utils.rad2deg)
        ans = utils.marge_zenithAng( post, lat, sunDec, sunRA, t, obsAng, solarOcclusionAng=solarOcclusionAng ) * utils.rad2deg
        zenithAng[lat==obsLat] = ans
        print >> file_obj, "%3.1f %8.3f+%8.3fj"%(lat*utils.rad2deg, ans.real, ans.imag)

    file_obj.close()

    #--------------------

    figname = 'sanityCheck_marge_zenithAng-%d.png'%(i)
    print "  "+figname
    ...
    ### plot zenithAng -> average value excluding regions that are unobservable...
    lalinf_plot.healpix_heatmap( zenithAng.real, cmap=plt.get_cmap('OrRd'), nest=False, vmin=0., vmax=180 )
    ...
    figname = 'sanityCheck_marge_zenithAng_SolarOcclusion-%d.png'%(i)
    print "  "+figname
    ...
    figname = "sanityCheck_marge_zenithAng_ObservableRegion-%d.png"%(i)
    print "  "+figname
